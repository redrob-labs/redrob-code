...
const repo = process.env.GH_REPO ?? "redrob-labs/redrob-code"
const bot = ["actions-user", "github-actions[bot]", "redrob", "redrob-agent[bot]"]
...
async function commits(from: string, to: string) {
  ...
  const log =
    await $`git log ${base}..${head} --format=%H -- packages/redrob packages/tui packages/sdk packages/plugin sdks/vscode github`.text()
  ...
  for (const hash of log.split("\n").filter(Boolean)) {
    ...
    for (const file of diff.split("\n").filter(Boolean)) {
      if (file.startsWith("packages/redrob/src/cli/cmd/") || file.startsWith("packages/tui/")) areas.add("tui")
      else if (file.startsWith("packages/redrob/")) areas.add("core")
