...
describe("redrob acp initialize/auth subprocess", () => {
  cliIt.live(
    ...
    ({ redrob }) =>
      Effect.gen(function* () {
        ...
        expect(initialized.agentInfo?.name).toBe("RedrobCode")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ redrob }) =>
      Effect.gen(function* () {
        const acp = yield* createAcpClient({ redrob })
        const initialized = yield* initialize(acp)

        expect(initialized.authMethods?.[0]?.id).toBe("redrob-login")
        expect(initialized.authMethods?.[0]?._meta?.["terminal-auth"]).toBeDefined()
        expect(yield* acp.request<AuthenticateResponse>("authenticate", { methodId: "redrob-login" })).toMatchObject({
          result: {},
        })
        ...
        expect(JSON.stringify(rejected.error)).not.toContain(process.env.REDROB_AUTH_CONTENT ?? "not-present")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ redrob }) =>
      Effect.gen(function* () {
        ...
        expect(initialized.result?.authMethods?.[0]?.id).toBe("redrob-login")
