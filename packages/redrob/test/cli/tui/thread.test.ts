...
describe("tui thread", () => {
  ...
  cliIt.live("rejects mini-only options without --mini", ({ redrob }) =>
    Effect.gen(function* () {
      const result = yield* redrob.spawn(["--replay-limit", "10"])

      redrob.expectExit(result, 1)
      expect(result.stderr).toContain("--replay-limit requires --mini")
    }),
  )

  cliIt.live("routes attached sessions to mini mode", ({ redrob }) =>
    Effect.gen(function* () {
      const result = yield* redrob.spawn(["attach", "http://127.0.0.1:1", "--mini"])
  ...
  cliIt.live("rejects network options in mini mode", ({ redrob }) =>
    Effect.gen(function* () {
      const result = yield* redrob.spawn(["--mini", "--port", "4096"])
