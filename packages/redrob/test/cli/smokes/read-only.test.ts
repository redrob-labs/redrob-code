...
describe("redrob read-only commands (smoke)", () => {
  // `mcp list` reads MCP server config and pings each one. With the empty
  // REDROB_CONFIG_CONTENT={} we provide, no servers should be configured
  // and the command should report that cleanly.
  cliIt.live(
    "mcp list: exits 0",
    ({ redrob }) =>
      Effect.gen(function* () {
        const r = yield* redrob.spawn(["mcp", "list"])
        redrob.expectExit(r, 0, "mcp list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ redrob }) =>
      Effect.gen(function* () {
        const r = yield* redrob.spawn(["providers", "list"])
        redrob.expectExit(r, 0, "providers list")
  ...
  cliIt.live(
    ...
    ({ redrob }) =>
      Effect.gen(function* () {
        const r = yield* redrob.spawn(["models"])
        redrob.expectExit(r, 0, "models")
  ...
  cliIt.live(
    ...
    ({ redrob }) =>
      Effect.gen(function* () {
        const r = yield* redrob.spawn(["agent", "list"])
        redrob.expectExit(r, 0, "agent list")
      }),
    60_000,
  )

  // `session list` reads the session DB. Fresh REDROB_TEST_HOME means
  // empty DB. Exit 0 with no sessions.
  cliIt.live(
    "session list: exits 0",
    ({ redrob }) =>
      Effect.gen(function* () {
        const r = yield* redrob.spawn(["session", "list"])
        redrob.expectExit(r, 0, "session list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ redrob }) =>
      Effect.gen(function* () {
        const r = yield* redrob.spawn(["stats"])
        redrob.expectExit(r, 0, "stats")
      }),
    60_000,
  )
  ...
  // in production it'd be a path under REDROB_TEST_HOME / XDG_DATA_HOME.
  // Accept either form — both prove the resolver ran without crashing.
  cliIt.live(
    "db path: exits 0 and prints a path or :memory:",
    ({ redrob }) =>
      Effect.gen(function* () {
        const r = yield* redrob.spawn(["db", "path"])
        redrob.expectExit(r, 0, "db path")
