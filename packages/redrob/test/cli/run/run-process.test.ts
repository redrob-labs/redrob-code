// Subprocess integration tests for `redrob run` (non-interactive mode).
// These exercise the real CLI binary against a TestLLMServer running in the
// same process. See `test/lib/cli-process.ts` for the harness — each test uses
// `redrob.run(message, opts?)` to spawn `bun src/index.ts run ...` with
// `REDROB_CONFIG_CONTENT` providing the test provider config inline.
...
describe("redrob run (non-interactive subprocess)", () => {
  ...
  cliIt.concurrent(
    ...
    ({ llm, redrob }) =>
      Effect.gen(function* () {
        ...
        const result = yield* redrob.run("use a tool", {
  ...
  cliIt.concurrent(
    ...
    ({ llm, redrob }) =>
      Effect.gen(function* () {
        ...
        const thinking = yield* redrob.run("think", { extraArgs: ["--thinking"] })
        redrob.expectExit(thinking, 0)
        expect(thinking.stdout).toBe("Thinking: considering\nanswer\n")

        yield* llm.reason("hidden", { text: "visible" })
        const plain = yield* redrob.run("think again")
        redrob.expectExit(plain, 0)
  ...
  cliIt.concurrent(
    ...
    ({ llm, redrob }) =>
      Effect.gen(function* () {
        ...
        const result = yield* redrob.run("say hi", { format: "json" })
        redrob.expectExit(result, 0)

        const events = redrob.parseJsonEvents(result.stdout)
  ...
  cliIt.concurrent(
    ...
    ({ redrob }) =>
      Effect.gen(function* () {
        const result = yield* redrob.run("use an unknown model", {
          model: "test/nonexistent-model",
          format: "json",
        })
  ...
  cliIt.concurrent(
    ...
    ({ llm, redrob }) =>
      Effect.gen(function* () {
        ...
        const result = yield* redrob.run("exercise json records", {
          format: "json",
  ...
  cliIt.concurrent(
    ...
    ({ llm, redrob }) =>
      Effect.gen(function* () {
        ...
        yield* llm.text("recovered")
        const result = yield* redrob.run("fail after output", { format: "json" })
  ...
  cliIt.concurrent(
    ...
    ({ home, llm, redrob }) =>
      Effect.gen(function* () {
        ...
        const denied = yield* redrob.run("request permission", { permission: { bash: "ask" } })
        redrob.expectExit(denied, 0)
        ...
        const allowed = yield* redrob.run("request permission", {
          permission: { bash: "ask" },
          extraArgs: ["--dangerously-skip-permissions"],
        })
        redrob.expectExit(allowed, 0)
        ...
        const explicitlyDenied = yield* redrob.run("request denied permission", {
          permission: { bash: "deny" },
          extraArgs: ["--dangerously-skip-permissions"],
        })
        redrob.expectExit(explicitlyDenied, 0)
  ...
  cliIt.live(
    ...
    ({ home, llm, redrob }) =>
      Effect.gen(function* () {
        ...
        const result = yield* redrob.run("read the attachment", {
  ...
  cliIt.concurrent(
    ...
    ({ home, redrob }) =>
      Effect.gen(function* () {
        const result = yield* redrob.run("read the directory", {
  ...
  cliIt.live(
    ...
    ({ llm, redrob }) =>
      Effect.gen(function* () {
        yield* llm.hang
        const run = yield* redrob.startRun("wait forever")
        yield* llm.wait(1)
        run.interrupt()
