...
import { RedrobClient, type GlobalEvent } from "@redrob-labs/sdk/v2"
import { createSessionTransport } from "@/cli/cmd/run/stream.transport"
import type { FooterApi, FooterEvent, LocalReplayRow, RunFilePart, StreamCommit } from "@/cli/cmd/run/types"

type EventStream = Awaited<ReturnType<RedrobClient["event"]["subscribe"]>>["stream"]
type GlobalEventStream = Awaited<ReturnType<RedrobClient["global"]["event"]>>["stream"]
type SdkEvent = EventStream extends AsyncGenerator<infer T, unknown, unknown> ? T : never
type SessionMessage = NonNullable<Awaited<ReturnType<RedrobClient["session"]["messages"]>>["data"]>[number]
type SessionChild = NonNullable<Awaited<ReturnType<RedrobClient["session"]["children"]>>["data"]>[number]
type SessionToolPart = Extract<SessionMessage["parts"][number], { type: "tool" }>
type SessionStatusMap = NonNullable<Awaited<ReturnType<RedrobClient["session"]["status"]>>["data"]>
...
function ok<T>(data: T) {
  return Promise.resolve({
    data,
    error: undefined,
    request: new Request("https://redrob.test"),
...
function sdk(
  input: {
    ...
    subscribe?: RedrobClient["event"]["subscribe"]
    globalEvent?: RedrobClient["global"]["event"]
    promptAsync?: RedrobClient["session"]["promptAsync"]
    status?: RedrobClient["session"]["status"]
    messages?: RedrobClient["session"]["messages"]
    children?: RedrobClient["session"]["children"]
    permissions?: RedrobClient["permission"]["list"]
    questions?: RedrobClient["question"]["list"]
  } = {},
) {
  const client = new RedrobClient()

  const subscribe: RedrobClient["event"]["subscribe"] = input.subscribe ?? (() => sse(input.stream ?? emptyStream()))
  const globalEvent: RedrobClient["global"]["event"] =
    input.globalEvent ?? (() => globalSse(input.globalStream ?? wrapGlobalStream(input.stream ?? emptyStream())))
  const promptAsync: RedrobClient["session"]["promptAsync"] = input.promptAsync ?? (() => ok(undefined))
  const status: RedrobClient["session"]["status"] = input.status ?? (() => ok({}))
  const messages: RedrobClient["session"]["messages"] = input.messages ?? (() => ok([]))
  const children: RedrobClient["session"]["children"] = input.children ?? (() => ok([]))
  const permissions: RedrobClient["permission"]["list"] = input.permissions ?? (() => ok([]))
  const questions: RedrobClient["question"]["list"] = input.questions ?? (() => ok([]))
