 ...
 *
 * The script intentionally isolates `REDROB_DB` before importing modules that touch
...
const scenarios: Scenario[] = [
  ...
  http.protected
    ...
    .seeded(() =>
      Effect.promise(() =>
        Bun.write(
          path.join(exerciseConfigDirectory, "redrob.jsonc"),
  ...
  http.protected.get("/path", "path.get").json(200, (body, ctx) => {
    object(body)
    check(body.directory === ctx.directory, "directory should resolve from x-redrob-directory")
    check(body.worktree === ctx.directory, "worktree should resolve from x-redrob-directory")
  }),
  ...
  http.protected
    ...
    .at((ctx) => ({
      ...
      headers: { ...ctx.headers(), "x-redrob-ticket": "1" },
    }))
    .json(404, object, "status"),
  http.protected
  ...
  http.protected
    ...
    .json(200, (body) => {
      ...
      // This exerciser does not load the built-in agent plugin, so the session has no
      // ruleset and PermissionV2 falls back to deny-all. The route still returns the
      // evaluated effect; a pending request is only created when that effect is "ask".
      check(
        body.data.effect === "deny" || body.data.effect === "ask",
        `permission create should return an evaluated effect, got ${JSON.stringify(body.data)}`,
      )
    }),
  http.protected
