// Subprocess test harness for the redrob CLI. Spawns the real binary against
...
//
// Configuration flows through redrob's built-in test affordances:
//   - REDROB_CONFIG_CONTENT      : provider config inline, no files to find
//   - REDROB_TEST_HOME           : pins os.homedir() → tmpdir
//   - REDROB_DISABLE_PROJECT_CONFIG : skip walking up for redrob.json
//   - REDROB_PURE                : skip external plugin discovery + install
//   - REDROB_DISABLE_AUTOUPDATE / AUTOCOMPACT / MODELS_FETCH : no background work
// Plus HOME / XDG_* pointing at the tmpdir for belt-and-suspenders isolation.
//
// Today only `redrob.run` is fully wired. The shape supports adding more
// builders (`redrob.serve(opts)`, `redrob.acp(opts)`, `redrob.auth(...)`)
...
const redrobRoot = path.resolve(import.meta.dir, "../../")
const cliEntry = path.join(redrobRoot, "src/index.ts")

// A `cliIt.concurrent` test cold-boots `bun run src/index.ts`, which costs about 1.7s on
// four cores and 3.0s on one. Ten of them starting at t=0 oversubscribe the runner, so the
// harness child timeout fires before any command finishes and a duration assertion measures
// queue depth instead of the command. Gate short-lived spawns to the machine's parallelism.
// Long-lived `startRun`/`serve`/`acp` handles are deliberately not gated: they live for the
...
function isolatedEnv(home: string, configJson: string): Record<string, string> {
  return {
    REDROB_TEST_HOME: home,
    HOME: home,
    XDG_CONFIG_HOME: path.join(home, ".config"),
    XDG_DATA_HOME: path.join(home, ".local/share"),
    XDG_STATE_HOME: path.join(home, ".local/state"),
    XDG_CACHE_HOME: path.join(home, ".cache"),
    REDROB_CONFIG_CONTENT: configJson,
    REDROB_DISABLE_PROJECT_CONFIG: "1",
    REDROB_PURE: "1",
    REDROB_DISABLE_AUTOUPDATE: "1",
    REDROB_DISABLE_AUTOCOMPACT: "1",
...
// Typed equivalent of constructing argv for `redrob run`. New flags should
...
// `redrob serve` is a long-lived process — it never exits on its own.
...
// `redrob acp` speaks newline-delimited JSON-RPC over stdin/stdout. It is
...
export type OpencodeCli = {
  ...
  // Spawn `redrob serve` and wait until it's listening. Long-lived: the
  ...
  // Spawn `redrob acp` and return a duplex JSON-RPC handle. Long-lived:
...
export function withCliFixture<A, E>(
  ...
  return Effect.gen(function* () {
    ...
    const spawn = Effect.fn("redrob.spawn")(function* (args: string[], opts?: SpawnOpts) {
      // The permit is taken before the clock starts so `durationMs` still measures the
      // command rather than how long it waited behind other spawns.
    ...
    const runOpts = (opts?: RunOpts): SpawnOpts | undefined => {
      ...
      return {
        ...
        env: {
          ...opts.env,
          REDROB_CONFIG_CONTENT: JSON.stringify({
    ...
    const startRun = Effect.fn("redrob.startRun")(function* (message: string, opts?: RunOpts) {
      const start = Date.now()
    ...
    const serve = Effect.fn("redrob.serve")(function* (opts?: ServeOpts) {
      const argv = ["serve"]
      ...
      //   "redrob server listening on http://<host>:<port>"
      ...
      const match = yield* Deferred.await(readyDeferred).pipe(
        Effect.timeoutOrElse({
          ...
          orElse: () =>
            Effect.fail(
              new Error(
                `redrob serve did not become ready within ${readyTimeoutMs}ms\n` +
    ...
    const acp = Effect.fn("redrob.acp")(function* (opts?: AcpOpts) {
      const argv = ["acp"]
    ...
    const redrob: OpencodeCli = { run, startRun, serve, acp, spawn, expectExit, parseJsonEvents }

    return yield* fn({ llm, home, redrob })
    // FetchHttpClient is provided so test bodies can `yield* HttpClient.HttpClient`
    // and hit endpoints on `redrob.serve()` without rolling their own fetch.
  }).pipe(
    Effect.provide(
      Layer.mergeAll(
        TestLLMServer.layer,
...
function expectExit(result: RunResult, expected: number, label = "redrob") {
...
// every call site. Use this for any test that needs the redrob CLI fixture.
//
...
// resources (e.g. `redrob.serve`) without an extra `Effect.scoped` wrapper —
