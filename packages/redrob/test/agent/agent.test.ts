...
it.instance("plan agent denies edits except .redrob/plans/*", () =>
  Effect.gen(function* () {
    ...
    expect(Permission.evaluate("edit", ".redrob/plans/foo.md", plan!.permission).action).toBe("allow")
  }),
)
...
it.instance(
  ...
  () =>
    Effect.gen(function* () {
      ...
      const skillDir = path.join(test.directory, ".redrob", "skill", "perm-skill")
...
`,
      ...
      const home = process.env.REDROB_TEST_HOME
      process.env.REDROB_TEST_HOME = test.directory
