...
import { HttpRecorder } from "@redrob-code/http-recorder"
import { HttpRecorderInternal } from "@redrob-code/http-recorder/internal"
import type { Hooks } from "@redrob-code/plugin"
...
import { LLMEvent, LLMResponse } from "@redrob-code/llm"
import { RequestExecutor } from "@redrob-code/llm/route"
...
const consoleURL = (connection: string) => `https://console.redrob.ai/proxy/connections/${connection}/v1`
...
function decodeRecordOpenAIOAuth() {
  const value = process.env.REDROB_RECORD_OPENAI_AUTH
...
const RECORDED_SCENARIOS = [
  {
    ...
    tags: ["redrob", "native", "tool-loop"],
    canRecord: () => Boolean(envValue("REDROB_RECORD_OPENAI_API_KEY", "OPENAI_API_KEY")),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.openai,
        name: "OpenAI",
        ...
        model,
        options: {
          apiKey: envValue("REDROB_RECORD_OPENAI_API_KEY", "OPENAI_API_KEY") ?? "fixture-openai-key",
  ...
  },
  {
    ...
    protocol: "openai-responses",
    tags: ["redrob", "native", "oauth", "tool-loop"],
  ...
  {
    id: "redrob-proxy",
    name: "RedrobCode proxy",
    providerID: ProviderV2.ID.redrob,
    modelID: "gpt-5.2-codex",
    cassette: "session/native-console-tool-loop",
    protocol: "openai-responses",
    tags: ["redrob", "native", "tool-loop"],
    canRecord: () => Boolean(process.env.REDROB_RECORD_CONSOLE_TOKEN && process.env.REDROB_RECORD_CONSOLE_ORG_ID),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.redrob,
        name: "Redrob Console",
        env: ["REDROB_CONSOLE_TOKEN"],
        npm: "@ai-sdk/openai-compatible",
        api: consoleURL(process.env.REDROB_RECORD_CONSOLE_CONNECTION ?? "fixture"),
        model,
        options: {
          apiKey: process.env.REDROB_RECORD_CONSOLE_TOKEN ?? "fixture-console-token",
          headers: { "x-org-id": process.env.REDROB_RECORD_CONSOLE_ORG_ID ?? "fixture-org" },
        },
      }),
  },
  {
    ...
    tags: ["redrob", "native", "tool-loop"],
    canRecord: () => Boolean(envValue("REDROB_RECORD_ANTHROPIC_API_KEY", "ANTHROPIC_API_KEY")),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.anthropic,
        name: "Anthropic",
        ...
        model,
        options: {
          apiKey: envValue("REDROB_RECORD_ANTHROPIC_API_KEY", "ANTHROPIC_API_KEY") ?? "fixture-anthropic-key",
...
const selectedScenarios = new Set(
  (envValue("REDROB_RECORDED_SCENARIO", "RECORDED_PROVIDER") ?? "")
    .split(",")
...
const recordError = (scenario: RecordedScenario) =>
  scenario.id === "openai-oauth"
    ? "Set REDROB_RECORD_OPENAI_AUTH to an OAuth auth JSON object in the recording environment."
...
// `LLMNativeRuntime.status` admits an OAuth credential only when the provider carries a
// `fetch` override, which an auth plugin's `loader` installs. The built-in ChatGPT/Codex
// plugin used to supply it; `802f785` deleted the plugin, so the scenario supplies the same
// hook here rather than resurrecting a login flow.
function oauthPluginLayer(scenario: RecordedScenario) {
...
function recordedNativeLLMLayer(scenario: RecordedScenario) {
  ...
  const oauthPlugin = oauthPluginLayer(scenario)
  // Only the HTTP client is recorded; RequestExecutor and the redrob LLM stack remain real.
  const metadata = {
    provider: scenario.providerID,
  ...
  return AppNodeBuilder.build(LayerNode.group([Provider.node, LLM.node]), [
    ...
    ...(oauthPlugin ? ([[Plugin.node, oauthPlugin]] as const) : []),
  ])
}
...
const writeConfig = (directory: string, scenario: RecordedScenario, model: ModelsDev.Provider["models"][string]) =>
  Effect.promise(() =>
    Bun.write(
      path.join(directory, "redrob.json"),
      JSON.stringify({ $schema: "https://code.redrob.ai/config.json", ...scenario.config(model) }),
    ),
  )
