...
import { LLMClient, RequestExecutor } from "@redrob-code/llm/route"
...
describe("session.llm.stream", () => {
  ...
  const redrobFixture = { providerID: "redrob-test", modelID: vivgridFixture.modelID }

  it.instance(
    "sends the parent session header for redrob providers",
    () =>
      Effect.gen(function* () {
        ...
        const resolved = yield* Provider.use.getModel(
          ProviderV2.ID.make(redrobFixture.providerID),
          ModelV2.ID.make(redrobFixture.modelID),
        )
        ...
        const user = {
          ...
          agent: agent.name,
          model: {
            providerID: ProviderV2.ID.make(redrobFixture.providerID),
            modelID: resolved.id,
          },
      ...
      }),
    {
      config: () => {
        const fixture = loadFixture(vivgridFixture.providerID, vivgridFixture.modelID)
        return {
          enabled_providers: [redrobFixture.providerID],
          provider: {
            [redrobFixture.providerID]: {
              name: "RedrobCode Test",
