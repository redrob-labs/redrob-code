...
import { SessionMessageTable } from "@redrob-code/core/session/sql"
...
const writeConfig = Effect.fn("test.writeConfig")(function* (dir: string, config: Partial<ConfigV1.Info>) {
  yield* writeText(
    path.join(dir, "redrob.json"),
    JSON.stringify({ $schema: "https://code.redrob.ai/config.json", ...config }),
  )
})
...
it.instance("loop recovers from provider overflow via auto-compaction with default (unset) compaction", () =>
  Effect.gen(function* () {
    // Regression guard for the intentional compaction predicate asymmetry:
    //   - proactive compaction stays OFF by default (compaction.auto unset)
    //   - but a hard provider ContextOverflowError still recovers by compacting
    //     when compaction.auto is unset, and only fails closed on explicit `auto: false`.
    // The sibling test above pins the explicit-false (fail-closed) behavior; this
    // one pins the default (fail-open / recover) behavior so the two cannot silently
    // converge.
    const { llm } = yield* useServerConfig(providerCfg)
    const prompt = yield* SessionPrompt.Service
    const sessions = yield* Session.Service
    ...
    // First request overflows; the queue then falls back to a normal "ok" reply,
    // so the emergency handler should compact and the retry should succeed.
    yield* llm.error(413, { error: { message: "request entity too large" } })
    yield* prompt.prompt({
      sessionID: chat.id,
      agent: "build",
...
it.instance("does not loop empty assistant turns for a simple reply", () =>
  Effect.gen(function* () {
    ...
    yield* llm.text("packages/redrob/src/session/processor.ts")
