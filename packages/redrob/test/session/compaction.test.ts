...
import { CONSOLE_CONTEXT_TOKENS, CONSOLE_OUTPUT_TOKENS } from "@redrob-code/core/plugin/provider/redrob"
...
import { LLMEvent, Usage } from "@redrob-code/llm"
...
describe("session.compaction.isOverflow", () => {
  ...
  // The limits the console catalog actually publishes, read from the shared constants rather than
  // retyped, so this fails if they ever go back to zero. Every Redrob model used to report a zero
  // context window, which short-circuits isOverflow() to false no matter how long the session ran.
  it.live(
    "compacts a Redrob console model at its published context window",
    provideTmpdirInstance(
      () =>
        Effect.gen(function* () {
          const compact = yield* SessionCompaction.Service
          const model = createModel({ context: CONSOLE_CONTEXT_TOKENS, output: CONSOLE_OUTPUT_TOKENS })
          const under = { input: 500_000, output: 5_000, reasoning: 0, cache: { read: 0, write: 0 } }
          expect(yield* compact.isOverflow({ tokens: under, model })).toBe(false)
          // usable = 1_000_000 - 32_000 = 968_000, so this crosses it.
          const over = { input: 960_000, output: 10_000, reasoning: 0, cache: { read: 0, write: 0 } }
