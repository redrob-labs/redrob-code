...
const paid = (providers: Record<string, { models: Record<string, { cost: { input: number } }> }>) => {
  const item = providers[ProviderV2.ID.make("redrob")]
...
it.instance("ModelNotFoundError suggests catalog models for unloaded providers", () =>
  Effect.gen(function* () {
    yield* remove("REDROB_API_KEY")
    const error = yield* Provider.use
      .getModel(ProviderV2.ID.redrob, ModelV2.ID.make("claude-haiku-fake-model"))
      .pipe(Effect.flip)
...
it.instance(
  ...
  {
    config: {
      provider: { "cloudflare-ai-gateway": { options: { metadata: { invoked_by: "test", project: "redrob" } } } },
    },
  },
)
...
it.effect("plugin config providers persist after instance dispose", () =>
  Effect.gen(function* () {
    ...
    const configDir = path.join(dir, ".redrob")
...
it.instance(
  ...
  Effect.gen(function* () {
    ...
    const configDir = path.join(instance.directory, ".redrob")
    const root = path.join(configDir, "plugin")
    yield* Effect.promise(() => mkdir(root, { recursive: true }))
    yield* Effect.promise(() => markPluginDependenciesReady(configDir))
    yield* Effect.promise(() =>
...
it.instance("redrob console provider seeds the primary console model", () =>
  Effect.gen(function* () {
    const providers = yield* list
    expect(providers[ProviderV2.ID.redrob]).toBeDefined()
    // The catalog fixture already carries a redrob provider, standing in for the console listing,
    // and that listing is authoritative — so the seed only backfills the primary model, `auto`.
    // `redrob-ai` and `redrob-translate` are retired ids, so the seed must never name them.
    const model = providers[ProviderV2.ID.redrob].models["auto"]
    expect(model).toBeDefined()
    expect(model.api.url).toBe("https://console.redrob.ai/api/backend/v1")
    expect(model.api.npm).toBe("@ai-sdk/openai-compatible")
    expect(providers[ProviderV2.ID.redrob].models["redrob-ai"]).toBeUndefined()
    expect(providers[ProviderV2.ID.redrob].models["redrob-translate"]).toBeUndefined()
  }),
)

// These two used to assert the opposite of what they assert now: with no credential the loader
// deleted every redrob model priced above 0, so `paid` came back 0. That rule came from providers
// that mix free and paid models and it does not describe the console, which has no free tier — one
// key gates all six ids. It also could not survive the listing's real per-million rates being
// projected: the V1 loader reads the auth.json credential while the catalog fetch reads the V2
// credential store, so a user who ran `redrob providers login` would have had a fully priced live
// catalog deleted out from under them and `redrob models` would have gone empty. What a credential
// changes is the placeholder api key, not which models are listed.
it.effect("redrob loader lists the same models with or without a config apiKey", () =>
  Effect.gen(function* () {
    const noneDir = yield* tmpdirScoped()
    const keyedDir = yield* tmpdirScoped({
      config: { provider: { redrob: { options: { apiKey: "test-key" } } } },
    })
    ...
    expect(none).toBe(keyedCount)
  }).pipe(provideMultiInstance),
)

it.effect("redrob loader lists the same models with or without an auth.json credential", () =>
  Effect.gen(function* () {
    const noneDir = yield* tmpdirScoped()
    const keyedDir = yield* tmpdirScoped()
    ...
    yield* Effect.acquireRelease(
      Effect.promise(() => Filesystem.write(authPath, JSON.stringify({ redrob: { type: "api", key: "test-key" } }))),
      () =>
