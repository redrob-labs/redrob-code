...
describe("ProviderTransform.options - setCacheKey", () => {
  ...
  // Console (`redrob*`) rides @ai-sdk/openai-compatible, which spreads unrecognized
  // providerOptions keys verbatim into the Chat Completions body. gpt-5.6-terra is the
  // default Console model, so a camelCase cache key here broke the first prompt of a
  // fresh install with "Unrecognized request argument supplied: promptCacheKey".
  for (const apiID of ["gpt-5.6-terra", "gpt-5.6-sol", "gpt-5"]) {
    test(`should send no cache key or Responses-only options for Console ${apiID}`, () => {
      const result = ProviderTransform.options({
        model: {
          ...mockModel,
          id: `redrob/${apiID}`,
          providerID: "redrob",
          api: { id: apiID, url: "https://console.redrob.ai/api/backend/v1", npm: "@ai-sdk/openai-compatible" },
        },
        sessionID,
        providerOptions: {},
      })
    ...
    const msgs = [
      ...
      { role: "assistant", content: text("assistant string") },
      {
        ...
        content: [
          ...
          { type: "tool-call", toolCallId: "call-1", toolName: "Read", input: { filePath: ".redrob/tool/emoji.ts" } },
          {
            type: "tool-result",
            toolCallId: "call-2",
            toolName: "Read",
...
describe("ProviderTransform.message - strip openai metadata when store=false", () => {
  ...
  test("uses the SDK package namespace rather than provider ID", () => {
    ...
    const result = ProviderTransform.message(msgs, consoleModel, { store: false }) as any[]
  ...
  test("preserves metadata using providerID key when store is false", () => {
    ...
    const result = ProviderTransform.message(msgs, redrobModel, { store: false }) as any[]

    expect(result[0].content[0].providerOptions?.redrob?.itemId).toBe("msg_123")
    expect(result[0].content[0].providerOptions?.redrob?.otherOption).toBe("value")
  })
  ...
  test("preserves itemId across all providerOptions keys", () => {
    ...
    expect(result[0].providerOptions?.redrob?.itemId).toBe("msg_redrob")
    expect(result[0].providerOptions?.extra?.itemId).toBe("msg_extra")
    expect(result[0].content[0].providerOptions?.openai?.itemId).toBe("msg_openai_part")
    expect(result[0].content[0].providerOptions?.redrob?.itemId).toBe("msg_redrob_part")
...
describe("ProviderTransform sampling defaults - DeepSeek", () => {
  ...
  test.each([
    ...
    ["redrob", "deepseek-v4-flash"],
    ["redrob-go", "deepseek-v4-flash"],
