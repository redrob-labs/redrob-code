...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "redrob.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "redrob.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "redrob.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "redrob.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["redrob.json", "redrob.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })
      ...
      expect(result).toEqual([
        ...
        path.join(project, "redrob.json"),
        path.join(project, "redrob.jsonc"),
      ])
    })
  })
