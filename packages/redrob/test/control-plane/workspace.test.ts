...
import { EventSequenceTable } from "@redrob-code/core/event/sql"
...
import { WorkspaceTable } from "@redrob-code/core/control-plane/workspace.sql"
...
const originalEnv = {
  REDROB_AUTH_CONTENT: process.env.REDROB_AUTH_CONTENT,
  REDROB_EXPERIMENTAL_WORKSPACES: process.env.REDROB_EXPERIMENTAL_WORKSPACES,
...
beforeEach(() => {
  restoreEnv()
  process.env.REDROB_EXPERIMENTAL_WORKSPACES = "true"
})
...
async function initGitRepo(dir: string) {
  ...
  await $`git config user.email "test@redrob.test"`.cwd(dir).quiet()
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const instance = yield* requireInstance
        const workspace = yield* Workspace.Service
        process.env.REDROB_AUTH_CONTENT = JSON.stringify({ test: { type: "api", key: "secret" } })
        process.env.OTEL_EXPORTER_OTLP_HEADERS = "authorization=otel"
        process.env.OTEL_EXPORTER_OTLP_ENDPOINT = "https://otel.test"
        process.env.OTEL_RESOURCE_ATTRIBUTES = "service.name=redrob-test"
        ...
        expect(JSON.parse(recorded.calls.create[0].env.REDROB_AUTH_CONTENT ?? "{}")).toEqual({
          test: { type: "api", key: "secret" },
        })
        expect(recorded.calls.create[0].env.REDROB_WORKSPACE_ID).toBe(workspaceID)
        expect(recorded.calls.create[0].env.REDROB_EXPERIMENTAL_WORKSPACES).toBe("true")
        ...
        expect(recorded.calls.create[0].env.OTEL_RESOURCE_ATTRIBUTES).toBe("service.name=redrob-test")
