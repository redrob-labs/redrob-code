...
- The server still lives in `packages/redrob`.
...
Until enough shared service code lives outside `packages/redrob`, a future
`packages/server` should either:

- own pure HttpApi contracts only, or
- accept host-provided services/layers/callbacks from `packages/redrob`

It should not import `packages/redrob` services while `packages/redrob`
...
3. Extract pure HttpApi contract modules into `packages/server` once the contract
   can compile without importing `packages/redrob` implementation details.
