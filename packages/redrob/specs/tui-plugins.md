...
- Author package entrypoint is `@redrob-code/plugin/tui`.
...
```json
{
  "$schema": "https://code.redrob.ai/tui.json",
  "theme": "smoke-theme",
  "leader_timeout": 2000,
  "keybinds": {
    "leader": "ctrl+x",
...
- Import types from `@redrob-code/plugin/tui`.
- `@redrob-code/plugin` exports `./tui` and declares optional peer deps on `@opentui/core` and `@opentui/solid`.

Minimal module shape:

```tsx
/** @jsxImportSource @opentui/solid */
import type { TuiPlugin, TuiPluginModule } from "@redrob-code/plugin/tui"
...
When `plugin` entries exist in a writable `.opencode` dir or `REDROB_CONFIG_DIR`, OpenCode installs `@redrob-code/plugin` into that dir and writes:
...
That is what makes local config-scoped plugins able to import `@redrob-code/plugin/tui`.
...
- For generic config-to-bindings helpers, import `createBindingLookup` from `@redrob-code/plugin/tui`.
...
- `--pure` / `REDROB_PURE` skips external TUI plugins only.
