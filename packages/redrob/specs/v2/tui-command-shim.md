...
Delete `packages/redrob/src/cli/cmd/tui/plugin/command-shim.ts`.

In `packages/redrob/src/cli/cmd/tui/plugin/api.tsx`, remove:

- the `createCommandShim` import
- the `command: createCommandShim(...)` field from `createTuiApi(...)`

In `packages/redrob/src/cli/cmd/tui/plugin/runtime.ts`, remove:
...
- `bun typecheck` in `packages/redrob`
- TUI plugin loader tests in `packages/redrob` if runtime plugin API wiring changed
