...
import { ConfigMCPV1 } from "@redrob-code/core/v1/config/mcp"
...
export const McpListCommand = effectCmd({
  ...
  handler: Effect.fn("Cli.mcp.list")(function* () {
    ...
    if (servers.length === 0) {
      ...
      prompts.outro("Add servers with: redrob mcp add")
      return
    }
...
export const McpAuthCommand = effectCmd({
  ...
  handler: Effect.fn("Cli.mcp.auth")(function* (args) {
    ...
    if (servers.length === 0) {
      ...
      prompts.log.info("Remote MCP servers support OAuth by default. Add a remote server in redrob.json:")
      prompts.log.info(`
  "mcp": {
    "my-server": {
      "type": "remote",
...
async function resolveConfigPath(baseDir: string, global = false) {
  // Check for existing config files (prefer .jsonc over .json, and check the `.redrob/`
  // subdirectory plus the legacy `.opencode/` one)
  const candidates = [path.join(baseDir, "redrob.json"), path.join(baseDir, "redrob.jsonc")]

  if (!global) {
    candidates.push(
      path.join(baseDir, ".redrob", "redrob.json"),
      path.join(baseDir, ".redrob", "redrob.jsonc"),
      path.join(baseDir, ".opencode", "redrob.json"),
      path.join(baseDir, ".opencode", "redrob.jsonc"),
    )
  }
  ...
  // Default to redrob.json if none exist
  return candidates[0]
}
    ...
    yield* Effect.promise(async () => {
      ...
      if (type === "local") {
        const command = await prompts.text({
          ...
          placeholder: "e.g., redrob x @modelcontextprotocol/server-filesystem",
          validate: (x) => (x && x.length > 0 ? undefined : "Required"),
        })
        ...
        const response = await fetch(serverConfig.url, {
          ...
          body: JSON.stringify({
            ...
            params: {
              ...
              capabilities: {},
              clientInfo: { name: "redrob-debug", version: InstallationVersion },
            },
            id: 1,
          }),
        })
