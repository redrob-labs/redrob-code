import type { PermissionV1 } from "@redrob-code/core/v1/permission"
import { FSUtil } from "@redrob-code/core/fs-util"
// CLI entry point for `redrob run` and `redrob --mini`.
//
// Handles three modes:
//   1. Non-interactive (default): sends a single prompt, streams events to
//      stdout, and exits when the session goes idle.
//   2. Interactive local (`redrob --mini`): boots the split-footer direct mode
//      with an in-process server (no external HTTP).
//   3. Interactive attach (redrob --mini --attach): connects to a running
//      redrob server and runs interactive mode against it.
//
...
import { createRedrobClient, type RedrobClient, type ToolPart } from "@redrob-labs/sdk/v2"
import { FormatError, FormatUnknownError } from "../error"
import { INTERACTIVE_INPUT_ERROR, resolveInteractiveStdin } from "./run/runtime.stdin"

type ModelInput = Parameters<RedrobClient["session"]["prompt"]>[0]["model"]
...
export const RunCommand = effectCmd({
  command: "run [message..]",
  describe: "run redrob with a message",
  ...
  builder: (yargs: Argv) =>
    yargs
      ...
      .option("attach", {
        type: "string",
        describe: "attach to a running redrob server (e.g., http://localhost:4096)",
      })
      .option("password", {
        alias: ["p"],
        type: "string",
        describe: "basic auth password (defaults to REDROB_SERVER_PASSWORD)",
      })
      .option("username", {
        alias: ["u"],
        type: "string",
        describe: "basic auth username (defaults to REDROB_SERVER_USERNAME or 'redrob')",
      })
      .option("dir", {
        type: "string",
  ...
  handler: Effect.fn("Cli.run")(function* (args) {
    ...
    yield* Effect.promise(async () => {
      ...
      async function session(sdk: RedrobClient): Promise<SessionInfo | undefined> {
        if (args.session) {
      ...
      async function share(sdk: RedrobClient, sessionID: string) {
      ...
      async function current(sdk: RedrobClient): Promise<string> {
        if (!args.attach) {
      ...
      async function attachAgent(sdk: RedrobClient) {
        if (!args.agent) return undefined
        const name = args.agent
      ...
      async function pickAgent(sdk: RedrobClient) {
        if (!args.agent) return undefined
        if (args.attach) {
      ...
      async function execute(sdk: RedrobClient) {
        ...
        async function loop(client: RedrobClient, events: Awaited<ReturnType<typeof sdk.event.subscribe>>) {
      ...
      const sdk = createRedrobClient({
        baseUrl: "http://redrob.internal",
        fetch: fetchFn,
        directory,
      })
      await execute(sdk)
