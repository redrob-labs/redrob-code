...
import { errorMessage } from "@redrob-code/tui/util/error"
...
import { win32InstallCtrlCGuard } from "@redrob-code/tui/terminal-win32"

declare global {
  const REDROB_WORKER_PATH: string
}
...
async function target() {
  if (typeof REDROB_WORKER_PATH !== "undefined") return REDROB_WORKER_PATH
...
export const TuiThreadCommand = cmd({
  command: "$0 [project]",
  describe: "start redrob tui",
  builder: (yargs) =>
    withNetworkOptions(yargs)
      .positional("project", {
        type: "string",
        describe: "path to start redrob in",
      })
      .option("model", {
        type: "string",
        alias: ["m"],
