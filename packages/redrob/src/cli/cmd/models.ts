...
export const ModelsCommand = effectCmd({
  ...
  builder: (yargs) =>
    yargs
      ...
      .option("refresh", {
        describe: "refresh the models list from the console",
        type: "boolean",
      }),
  handler: Effect.fn("Cli.models")(function* (args) {
    ...
    const ids = Object.keys(providers).sort((a, b) => {
      const aIsRedrob = a.startsWith("redrob")
      const bIsRedrob = b.startsWith("redrob")
      if (aIsRedrob && !bIsRedrob) return -1
      if (!aIsRedrob && bIsRedrob) return 1
