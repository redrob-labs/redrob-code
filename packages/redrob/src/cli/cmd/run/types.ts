// Shared type vocabulary for the direct interactive mode (`redrob --mini`).
//
...
type PromptModel = Parameters<RedrobClient["session"]["prompt"]>[0]["model"]
type PromptInput = Parameters<RedrobClient["session"]["prompt"]>[0]

export type RunPromptPart = NonNullable<PromptInput["parts"]>[number]

export type RunCommand = NonNullable<Awaited<ReturnType<RedrobClient["command"]["list"]>>["data"]>[number]

export type RunProvider = NonNullable<Awaited<ReturnType<RedrobClient["provider"]["list"]>>["data"]>["all"][number]
...
export type RunAgent = NonNullable<Awaited<ReturnType<RedrobClient["app"]["agents"]>>["data"]>[number]

type RunResourceMap = NonNullable<Awaited<ReturnType<RedrobClient["experimental"]["resource"]["list"]>>["data"]>
...
export type PermissionReply = Parameters<RedrobClient["permission"]["reply"]>[0]

export type QuestionReply = Parameters<RedrobClient["question"]["reply"]>[0]

export type QuestionReject = Parameters<RedrobClient["question"]["reject"]>[0]
