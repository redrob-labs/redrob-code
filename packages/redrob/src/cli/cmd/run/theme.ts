...
import type { TuiThemeCurrent } from "@redrob-code/plugin/tui"
...
// The seed palette behind RUN_THEME_FALLBACK, used only when the terminal refuses to report its
// palette. Each entry keeps the ANSI slot it always had, because on a terminal that does answer, the
// slot is what gets drawn and those slots were picked for legibility across terminal palettes - cyan
// rather than blue for the highlight, bright black for muted. Only the RGB fallback behind each slot
// is the brand's, so the one case where no palette exists paints the brand's colours instead of
// Tailwind's defaults. Slot 6 pairs with the brand's cyan for the same reason generateSystem maps
// ANSI cyan to primary: the fallback has to agree with the path it stands in for.
const seed = {
  highlight: RGBA.fromIndex(6, rgba(Brand.teal3)),
  muted: RGBA.fromIndex(8, rgba(Brand.gray6)),
  text: RGBA.defaultForeground(rgba(Brand.gray1)),
  panel: rgba(Brand.gray9),
  success: RGBA.fromIndex(2, rgba(Brand.green3)),
  warning: RGBA.fromIndex(3, rgba(Brand.orange3)),
  error: RGBA.fromIndex(1, rgba(Brand.red3)),
}
...
export async function resolveRunTheme(renderer: CliRenderer): Promise<RunTheme> {
  try {
    ...
    const shared = await import("@redrob-code/tui/context/theme")
