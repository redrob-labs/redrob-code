...
export class Service extends Context.Service<Service, Interface>()("@redrob/TuiConfig") {}
...
const loadState = Effect.fn("TuiConfig.loadState")(function* (ctx: { directory: string }) {
  ...
  const load = (text: string, configFilepath: string): Effect.Effect<Info> =>
    Effect.gen(function* () {
      ...
      // (mirroring the old redrob.json shape) still get their settings applied.
  ...
  // Every config dir we may read from: global config dir, any `.redrob` (or legacy
  // `.opencode`) folders between cwd and home, and REDROB_CONFIG_DIR.
  const directories = yield* ConfigPaths.directories(ctx.directory)
  yield* Effect.promise(() => migrateTuiConfig({ directories, cwd: ctx.directory }))

  const projectFiles = Flag.REDROB_DISABLE_PROJECT_CONFIG ? [] : yield* ConfigPaths.files("tui", ctx.directory)
  ...
  // 2. Explicit REDROB_TUI_CONFIG override, if set.
  if (Flag.REDROB_TUI_CONFIG) {
    const configFile = Flag.REDROB_TUI_CONFIG
  ...
  // 4. `.redrob` (and legacy `.opencode`) directories, plus REDROB_CONFIG_DIR, discovered
  // while walking up the tree. Also returned below so callers can install plugin
  // dependencies from each location.
  const dirs = unique(directories).filter(ConfigPaths.isProjectDirectory)
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const deps = pluginTypes
      ? yield* Effect.forEach(data.dirs, (dir) => npm.install(dir, { add: [pluginTypes] }).pipe(Effect.forkScoped), {
