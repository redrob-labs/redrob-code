...
import { optional } from "@redrob-code/core/schema"
...
import {
  ...
  CONSOLE_PACKAGE,
  CONSOLE_URL,
} from "@redrob-code/core/plugin/provider/redrob"
import type { ConsoleModelInfo } from "@redrob-code/core/plugin/provider/redrob-constants"
...
// Authoritative console.redrob.ai provider constants are imported from
// @redrob-code/core/plugin/provider/redrob (RedrobPlugin), which registers the same provider
// into the V2 catalog. Sharing the import makes core the single source of truth so the V1 seed
// and the V2 registration cannot drift. The V1 Provider.Service does not consume the V2 catalog,
// so the redrob provider is seeded directly into the V1 database (see redrobSeed) to keep
// `redrob models` listing when the dynamic catalog sourced from the console /models endpoint is
// empty or unreachable (no key set or the request failed).
    ...
    async pull(ctrl) {
      const part = await new Promise<Awaited<ReturnType<typeof reader.read>>>((resolve, reject) => {
        const id = setTimeout(() => {
          ...
          ctl.abort(err)
          // cancel() rejects because the abort above already tore the fetch down. Discarding
          // that rejection with `void` surfaces it as an unhandled rejection instead.
...
const BUNDLED_PROVIDERS: Record<string, () => Promise<(opts: any) => BundledSDK>> = {
  ...
  "@ai-sdk/github-copilot": () =>
    import("@redrob-code/core/github-copilot/copilot-provider").then((m) => m.createOpenaiCompatible),
...
function custom(dep: CustomDep): Record<string, CustomLoader> {
  return {
    ...
    redrob: Effect.fnUntraced(function* (input: Info) {
      ...
      const ok =
        ...
        Boolean((yield* dep.config()).provider?.["redrob"]?.options?.apiKey)

      // Without a credential the SDK still needs a non-empty apiKey to instantiate, so it gets a
      // placeholder. What used to happen here was that every model priced above 0 was deleted from
      // the catalog, a rule inherited from providers that mix free and paid models. The console has
      // no free tier — one key gates all six ids — so a price says nothing about whether to list a
      // model, and now that the listing's real rates are projected that rule would have deleted all
      // six for every keyless user, leaving `redrob models` empty.
      return {
    ...
    "amazon-bedrock": Effect.fnUntraced(function* () {
      ...
      return {
        ...
        async getModel(sdk: any, modelID: string, options?: Record<string, any>, model?: Model) {
          ...
          // 1. options.region from redrob.json provider config
    ...
    llmgateway: () =>
      Effect.succeed({
        ...
        options: {
          headers: {
            "HTTP-Referer": "https://code.redrob.ai/",
            "X-Title": "redrob",
            "X-Source": "redrob",
          },
        },
      ...
      Effect.succeed({
        ...
        options: {
          headers: {
            "HTTP-Referer": "https://code.redrob.ai/",
            "X-Title": "redrob",
            "X-BILLING-INVOKE-ORIGIN": "RedrobCode",
          },
        },
      }),
    vercel: () =>
    ...
    gitlab: Effect.fnUntraced(function* (input: Info) {
      ...
      const aiGatewayHeaders = {
        "User-Agent": `redrob/${InstallationVersion} gitlab-ai-provider/${GITLAB_PROVIDER_VERSION} (${os.platform()} ${os.release()}; ${os.arch()})`,
    ...
    "cloudflare-workers-ai": Effect.fnUntraced(function* (input: Info) {
      ...
      return {
        ...
        options: {
          apiKey,
          headers: {
            "User-Agent": `redrob/${InstallationVersion} cloudflare-workers-ai (${os.platform()} ${os.release()}; ${os.arch()})`,
          },
        },
        async getModel(sdk: any, modelID: string) {
          return sdk.languageModel(modelID)
    ...
    "cloudflare-ai-gateway": Effect.fnUntraced(function* (input: Info) {
      ...
      if (!apiToken) {
        throw new Error(
          "CLOUDFLARE_API_TOKEN (or CF_AIG_TOKEN) is required for Cloudflare AI Gateway. " +
            "Set it via environment variable or run `redrob auth cloudflare-ai-gateway`.",
        )
      }
      ...
      const opts = {
        ...
        headers: {
          "User-Agent": `redrob/${InstallationVersion} cloudflare-ai-gateway (${os.platform()} ${os.release()}; ${os.arch()})`,
        },
      }
    ...
    "snowflake-cortex": Effect.fnUntraced(function* (input: Info) {
      ...
      if (!account || !token) {
        ...
        return {
          ...
          async getModel() {
            throw new Error(
              `Snowflake Cortex: missing credentials (${missing}). Provide a bearer token (OAuth, JWT, or PAT) via env var, redrob auth, or provider options.`,
            )
          },
        }
      }
...
export class Service extends Context.Service<Service, Interface>()("@redrob/Provider") {}
...
// Build the authoritative console.redrob.ai provider Info for the V1 database from the shared
// CONSOLE_MODELS list. Mirrors the values registered by RedrobPlugin into the V2 catalog: no rate,
// because only the console's live listing can supply one, and the verified context/output limits,
// because a zero context window makes `isOverflow()` unconditionally false and auto-compaction
// never runs.
function redrobSeedModel(model: ConsoleModelInfo): Model {
  return {
    id: ModelV2.ID.make(model.id),
    providerID: ProviderV2.ID.redrob,
    name: model.name,
    api: { id: model.id, url: CONSOLE_URL, npm: CONSOLE_PACKAGE },
    capabilities: {
      temperature: false,
    ...
    headers: {},
    release_date: "",
    // No effort variants: the console's thinking control is a top-level `thinking` level, not
    // OpenAI `reasoning_effort`, and its chat endpoint rejects fields it does not whitelist.
    variants: {},
  }
}

function redrobSeed(): Info {
  return {
    id: ProviderV2.ID.redrob,
    name: "Redrob",
    source: "custom",
    env: ["REDROB_API_KEY"],
    options: {},
    models: Object.fromEntries(CONSOLE_MODELS.map((model) => [model.id, redrobSeedModel(model)])),
  }
}
  ...
  Effect.gen(function* () {
    ...
    const state = yield* InstanceState.make<State>(() =>
      Effect.gen(function* () {
        ...
        // Seed the authoritative console.redrob.ai provider so `redrob models` lists every served
        // console model (`redrob/auto` and the rest of CONSOLE_MODELS) even when the dynamic
        // catalog from the console /models endpoint is empty or unreachable (no key set or the
        // request failed). Additive: never overwrite an existing redrob catalog (e.g. the dynamic
        // models); only fill in the missing pieces.
        // When a redrob catalog already exists it came from the console listing, which is
        // authoritative, so only the primary model is backfilled — a model the console stopped
        // advertising must not be resurrected from the static list.
        if (!database[ProviderV2.ID.redrob]) {
          database[ProviderV2.ID.redrob] = redrobSeed()
        } else if (!database[ProviderV2.ID.redrob].models[CONSOLE_MODEL.id]) {
          database[ProviderV2.ID.redrob].models[CONSOLE_MODEL.id] = redrobSeedModel(CONSOLE_MODEL)
        }
    ...
    const getSmallModel = Effect.fn("Provider.getSmallModel")(function* (providerID: ProviderV2.ID) {
      ...
      const priority = providerID.startsWith("redrob")
        ? ["gpt-nano"]
