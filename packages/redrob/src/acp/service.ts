...
export const AuthMethodID = "redrob-login"
...
export class Service extends Context.Service<Service, Interface>()("@redrob/ACP/Service") {}
...
export function make(input: {
  ...
  const initialize = Effect.fn("ACP.initialize")(function* (params: InitializeRequest) {
    ...
    const authMethod: AuthMethod = {
      description: "Run `redrob auth login` in the terminal to enter your API key",
      name: "Enter Redrob API key",
      id: AuthMethodID,
    }
...
function makeDirectoryService(sdk: RedrobClient) {
...
function makeUsageService(sdk: RedrobClient) {
...
async function loadDirectorySnapshot(sdk: RedrobClient, directory: string) {
...
function promptErrorMessage(error: AssistantError) {
  ...
  return "RedrobCode prompt failed"
}
...
function fromUnknownError(error: unknown, service?: string): Error {
  ...
  return new ACPError.ServiceFailureError({ safeMessage: "RedrobCode service failure", service })
}
