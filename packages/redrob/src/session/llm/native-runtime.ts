...
import type { LLMClientShape } from "@redrob-code/llm/route"
...
function statusWithFetch(
  ...
  if (providerID !== "openai" && providerID !== "anthropic" && !providerID.startsWith("redrob"))
    return { type: "unsupported", reason: "provider is not openai, redrob, or anthropic" }
...
export function stream(input: StreamInput): StreamResult {
  ...
  // Integration point with @redrob-code/llm: native-request lowers session data
...
export function nativeTools(tools: Record<string, Tool>, input: Pick<StreamInput, "messages" | "abort">) {
  return Object.fromEntries(
    Object.entries(tools).map(([name, item]) => [
      name,
      // Tool execution remains redrob-owned. The native runtime only adapts
      // the @redrob-code/llm tool call back into the AI SDK Tool.execute shape.
      NativeTool.make({
