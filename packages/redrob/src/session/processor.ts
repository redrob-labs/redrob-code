...
import { Usage, type LLMEvent } from "@redrob-code/llm"
...
export class Service extends Context.Service<Service, Interface>()("@redrob/SessionProcessor") {}
    ...
    const create = Effect.fn("SessionProcessor.create")(function* (input: Input) {
      ...
      const halt = Effect.fn("SessionProcessor.halt")(function* (e: unknown) {
        ...
        if (SessionV1.ContextOverflowError.isInstance(error)) {
          // Compaction predicate asymmetry (intentional):
          //   - Proactive compaction (overflow.ts `isOverflow`) only fires when the user
          //     opts in with `compaction.auto === true`, so the DEFAULT (unset) never
          //     compacts pre-emptively. That satisfies the "compaction off by default"
          //     requirement.
          //   - This emergency path runs only after the model has already rejected an
          //     over-length request (a hard ContextOverflowError). Here the DEFAULT (unset)
          //     still recovers by compacting to avoid silently losing the session, and only
          //     an EXPLICIT `compaction.auto === false` fails closed with an error.
          // The two states therefore diverge on the hard-overflow path on purpose: unset =
          // fail-open (recover), explicit false = fail-closed (surface the error).
