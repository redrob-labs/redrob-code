...
const LegacyComponentDescriptions: Record<string, string> = {
  LogLevel: "Log level",
  ServerConfig: "Server configuration for redrob serve and web commands",
...
function fixSelfReferencingComponents(spec: OpenApiSpec) {
  ...
  const raw: OpenApiSpec = OpenApi.fromApi(RedrobHttpApi)
...
export const PublicApi = RedrobHttpApi.annotateMerge(
  OpenApi.annotations({
    title: "redrob",
    version: "1.0.0",
    description: "redrob api",
