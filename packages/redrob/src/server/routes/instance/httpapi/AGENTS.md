...
Use raw `HttpRouter.use(...)` only for routes outside the declared API surface, such as `docRoute`, which serves `GET /doc` from `OpenApi.fromApi(PublicApi)`. Do not add a catch-all `"*" "/*"` route. The server used to serve a web UI from one; that surface was removed when Redrob Work became the sole GUI, and unmatched paths are expected to 404.
