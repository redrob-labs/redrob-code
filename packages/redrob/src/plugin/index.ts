...
import { RedrobAuthPlugin } from "./redrob"
...
export class Service extends Context.Service<Service, Interface>()("@redrob/Plugin") {}

// Built-in plugins that are directly imported (not installed from npm)
function internalPlugins(): PluginInstance[] {
  return [
    // First, so device connect is the first thing offered for the console provider: it is the only
    // path that gets a workspace key onto this machine without a person handling the secret.
    RedrobAuthPlugin,
    CopilotAuthPlugin,
    ModalPlugin,
    GitlabAuthPlugin,
