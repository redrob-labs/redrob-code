...
export const DEPRECATED_PLUGIN_PACKAGES = ["redrob-openai-codex-auth", "redrob-copilot-auth"]
...
export async function checkPluginCompatibility(target: string, redrobVersion: string, pkg?: PluginPackage) {
  if (!semver.valid(redrobVersion) || semver.major(redrobVersion) === 0) return
  const hit = pkg ?? (await readPluginPackage(target).catch(() => undefined))
  if (!hit) return
  const engines = hit.json.engines
  if (!isRecord(engines)) return
  const range = engines.redrob
  if (typeof range !== "string") return
  if (!semver.satisfies(redrobVersion, range)) {
    throw new Error(`Plugin requires redrob ${range} but running ${redrobVersion}`)
  }
}
