...
import {
  ...
  type TuiSlotPlugin,
  type TuiTheme,
} from "@redrob-code/plugin/tui"
import path from "path"
import { fileURLToPath } from "url"
import { TuiConfig } from "@/config/tui"
import { AppNodeBuilder } from "@redrob-code/core/effect/app-node-builder"
import { errorData, errorMessage } from "@redrob-code/tui/util/error"
...
import { hasTheme, upsertTheme } from "@redrob-code/tui/context/theme"
...
import type { HostPluginApi, HostSlots } from "@redrob-code/tui/plugin/slots"
import { ConfigPlugin } from "@/config/plugin"
import { ConfigPluginV1 } from "@redrob-code/core/v1/config/plugin"
import { createCommandShim } from "@redrob-code/tui/plugin/command-shim"
import { RuntimeFlags } from "@/effect/runtime-flags"
import { Effect } from "effect"
import { createPluginRuntime, type PluginRuntime, type TuiPluginHost } from "@redrob-code/tui/plugin/runtime"
...
function createThemeInstaller(
  ...
  return async (file) => {
    ...
    // A legacy `.opencode` config directory keeps its own `themes/`; everything else
    // writes under `.redrob`.
    const local_dir = [".redrob", ".opencode"].includes(path.basename(source_dir))
      ? path.join(source_dir, "themes")
      : path.join(source_dir, ".redrob", "themes")
...
function defaultPluginOrigin(state: RuntimeState, spec: string): ConfigPlugin.Origin {
  return {
    spec,
    scope: "local",
    source: state.api.state.path.config || path.join(state.directory, ".redrob", "tui.json"),
  }
}
...
async function load(input: {
  ...
  try {
    ...
    const records = Flag.REDROB_PURE ? [] : pluginOrigins
    if (Flag.REDROB_PURE && pluginOrigins.length) {
    }
