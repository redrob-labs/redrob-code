...
export type PatchDeps = {
  ...
  files: (dir: string, name: "redrob" | "tui") => string[]
}
...
function patchDir(input: PatchInput) {
  ...
  // New files go to `.redrob`; `.opencode` stays readable but is never written to.
  return path.join(root, ".redrob")
}

function patchName(kind: Kind): "redrob" | "tui" {
  if (kind === "server") return "redrob"
  return "tui"
}
