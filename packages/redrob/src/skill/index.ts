...
import { FrontmatterError } from "@redrob-code/core/v1/config/error"
import { ConfigMarkdown } from "@/config/markdown"
import { RuntimeFlags } from "@/effect/runtime-flags"
import { Glob } from "@redrob-code/core/util/glob"
...
const REDROB_SKILL_PATTERN = "{skill,skills}/**/SKILL.md"
const SKILL_PATTERN = "**/SKILL.md"

// Built-in skill that ships with redrob. The model's intuition for what a
// redrob.json should look like is often wrong, and redrob hard-fails on
// invalid config, so users hit cryptic startup errors. Loading this skill
// when the model is asked to touch redrob's own config files gives it the
// actual schemas instead of guesses.
const CUSTOMIZE_REDROB_SKILL_NAME = "customize-redrob"
const CUSTOMIZE_REDROB_SKILL_DESCRIPTION =
  "Use ONLY when the user is editing or creating redrob's own configuration: redrob.json, redrob.jsonc, files under .redrob/, or files under ~/.config/redrob/. Also use when creating or fixing redrob agents, subagents, skills, plugins, MCP servers, or permission rules. Do not use for the user's own application code, or for any project that is not configuring redrob itself."
const CUSTOMIZE_REDROB_SKILL_BODY = SkillPlugin.CustomizeRedrobContent
...
const discoverSkills = Effect.fnUntraced(function* (
  ...
  for (const dir of configDirs) {
    yield* scan(state, dir, REDROB_SKILL_PATTERN)
  }
...
export class Service extends Context.Service<Service, Interface>()("@redrob/Skill") {}
  ...
  Effect.gen(function* () {
    ...
    const state = yield* InstanceState.make(
      Effect.fn("Skill.state")(function* () {
        ...
        s.skills[CUSTOMIZE_REDROB_SKILL_NAME] = {
          name: CUSTOMIZE_REDROB_SKILL_NAME,
          description: CUSTOMIZE_REDROB_SKILL_DESCRIPTION,
          location: "<built-in>",
          content: CUSTOMIZE_REDROB_SKILL_BODY,
        }
