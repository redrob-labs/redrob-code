...
import { AccountStateTable, AccountTable } from "@redrob-code/core/account/sql"
...
export class Service extends Context.Service<Service, Interface>()("@redrob/AccountRepo") {}
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    // The credential is an API key that never expires or refreshes. The legacy
    // refresh_token/token_expiry columns are kept to avoid a DB migration: we mirror
    // the API key into refresh_token (which is NOT NULL) and leave token_expiry null.
          ...
          Effect.gen(function* () {
            ...
            yield* tx
              ...
              .values({
                ...
                refresh_token: RefreshToken.make(input.accessToken),
                token_expiry: null,
              })
              .onConflictDoUpdate({
