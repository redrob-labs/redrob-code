...
export interface Interface {
  readonly client: () => Effect.Effect<ReturnType<typeof createRedrobClient>, unknown>
...
export class Service extends Context.Service<Service, Interface>()("@redrob/cli/Daemon") {}
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const createClient = Effect.fnUntraced(function* (url: string) {
      return createRedrobClient({ baseUrl: url, headers: ServerAuth.headers({ password: yield* password() }) })
    })
    ...
    const client = Effect.fn("cli.daemon.client")(function* () {
      ...
      return createRedrobClient({ baseUrl: connection.url, headers: connection.headers })
    })
