...
console.log("🚀 Starting redrob server...")
const redrob = await createRedrob({
  port: 0,
})
console.log("✅ redrob server ready")

const sessions = new Map<string, { client: any; server: any; sessionId: string; channel: string; thread: string }>()
void (async () => {
  const events = await redrob.client.event.subscribe()
...
app.message(async ({ message, say }) => {
  ...
  if (!session) {
    console.log("🆕 Creating new redrob session...")
    const { client, server } = redrob
    ...
    console.log("✅ Created redrob session:", createResult.data.id)
  ...
  console.log("📝 Sending to redrob:", message.text)
  const result = await session.client.session.prompt({
    path: { id: session.sessionId },
    body: { parts: [{ type: "text", text: message.text }] },
  })

  console.log("📤 redrob response:", JSON.stringify(result, null, 2))
