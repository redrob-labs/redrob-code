...
import { PromptInput } from "@redrob-code/schema/prompt-input"
import { Session } from "@redrob-code/schema/session"
import { Project } from "@redrob-code/schema/project"
import { AbsolutePath, NonNegativeInt, PositiveInt, RelativePath, statics } from "@redrob-code/schema/schema"
...
import { SessionEvent } from "@redrob-code/schema/session-event"
    ...
    .add(
      HttpApiEndpoint.get("session.active", "/api/session/active", {
        ...
        OpenApi.annotations({
          ...
          description:
            "Retrieve foreground Session drains currently owned by this RedrobCode process. Sessions absent from the result are inactive.",
        }),
      ),
    )
    .add(
      ...
      HttpApiEndpoint.post("session.interrupt", "/api/session/:sessionID/interrupt", {
        ...
        .annotateMerge(
          OpenApi.annotations({
            ...
            description: "Interrupt active execution owned by this RedrobCode process. Idle interruption is a no-op.",
          }),
        ),
    )
    .add(
