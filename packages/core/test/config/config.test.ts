...
import { ConfigProvider } from "@redrob-code/core/config/provider"
...
import { ConfigMigrateV1 } from "@redrob-code/core/v1/config/migrate"
...
import { Project } from "@redrob-code/core/project"
...
describe("Config", () => {
  ...
  it.live("loads redrob JSON and JSONC files from lowest to highest priority", () =>
    Effect.acquireRelease(
      Effect.promise(() => tmpdir()),
      (tmp) => Effect.promise(() => tmp[Symbol.asyncDispose]()),
    ).pipe(
      Effect.flatMap((tmp) =>
        Effect.gen(function* () {
          yield* Effect.promise(() =>
            Promise.all([
              fs.writeFile(
                path.join(tmp.path, "redrob.json"),
                JSON.stringify({ $schema: "base", providers: { base: provider } }),
              ),
              fs.writeFile(
                path.join(tmp.path, "redrob.jsonc"),
                `{
          ...
          return yield* Effect.gen(function* () {
            ...
            expect(documents[0]?.path).toBe(path.join(tmp.path, "redrob.json"))
            expect(documents[1]?.info.providers?.last).toBeInstanceOf(ConfigProvider.Info)

            yield* Effect.promise(() =>
              fs.writeFile(path.join(tmp.path, "redrob.jsonc"), JSON.stringify({ $schema: "changed" })),
            )
            expect(
    ...
    Effect.acquireRelease(
      ...
      Effect.flatMap((tmp) =>
        Effect.gen(function* () {
          yield* Effect.promise(() =>
            Promise.all([
              fs.writeFile(path.join(tmp.path, "redrob.json"), JSON.stringify({ $schema: "base" })),
              fs.writeFile(path.join(tmp.path, "redrob.jsonc"), "{ invalid"),
            ]),
          )
          return yield* Effect.gen(function* () {
            const config = yield* Config.Service
      ...
      Effect.flatMap((tmp) => {
        ...
        return Effect.gen(function* () {
          yield* Effect.promise(async () => {
            ...
            await Promise.all([
              fs.writeFile(path.join(tmp.path, "redrob.json"), JSON.stringify({ $schema: "outside" })),
              fs.writeFile(path.join(global, "redrob.json"), JSON.stringify({ $schema: "global" })),
              fs.writeFile(path.join(root, "redrob.json"), JSON.stringify({ $schema: "root" })),
              fs.writeFile(path.join(parent, "redrob.jsonc"), JSON.stringify({ $schema: "parent" })),
              fs.writeFile(path.join(directory, "redrob.json"), JSON.stringify({ $schema: "directory" })),
              fs.writeFile(path.join(root, ".opencode", "redrob.json"), JSON.stringify({ $schema: "root-dot" })),
              fs.writeFile(
                path.join(directory, ".opencode", "redrob.jsonc"),
  ...
  it.live("loads .redrob configuration, and lets it override a co-located legacy .opencode", () =>
    Effect.acquireRelease(
      Effect.promise(() => tmpdir()),
      (tmp) => Effect.promise(() => tmp[Symbol.asyncDispose]()),
    ).pipe(
      Effect.flatMap((tmp) => {
        ...
        return Effect.gen(function* () {
          yield* Effect.promise(async () => {
            await fs.mkdir(global, { recursive: true })
            await fs.mkdir(directory, { recursive: true })
            // Both names in the SAME directory, which is the case that decides precedence.
            await fs.mkdir(path.join(root, ".opencode"), { recursive: true })
            await fs.mkdir(path.join(root, ".redrob"), { recursive: true })
            // And .redrob alone further down, which is what `redrob mcp add` writes.
            await fs.mkdir(path.join(directory, ".redrob"), { recursive: true })
            await Promise.all([
              fs.writeFile(path.join(global, "redrob.json"), JSON.stringify({ $schema: "global" })),
              fs.writeFile(path.join(root, ".opencode", "redrob.json"), JSON.stringify({ $schema: "root-legacy" })),
              fs.writeFile(path.join(root, ".redrob", "redrob.json"), JSON.stringify({ $schema: "root-redrob" })),
              fs.writeFile(
                path.join(directory, ".redrob", "redrob.jsonc"),
                JSON.stringify({ $schema: "directory-redrob" }),
              ),
            ])
          })
          ...
          return yield* Effect.gen(function* () {
            ...
            const entries = yield* config.entries()
            const documents = entries.filter((entry) => entry.type === "document")
            const schemas = documents.map((document) => document.info.$schema)

            // Discovery looked only in `.opencode` before this: the config FILE names had been
            // renamed to redrob.json and the FOLDER list had not. So a config written to
            // `.redrob/redrob.json` -- where `redrob mcp add` puts it, and where the config skill
            // tells the agent redrob's own settings live -- was silently ignored. No error, no
            // warning, no effect.
            expect(schemas).toContain("root-redrob")
            expect(schemas).toContain("directory-redrob")

            // The legacy name still loads, so an existing checkout keeps working.
            expect(schemas).toContain("root-legacy")

            // And `.redrob` is applied after `.opencode`, so a co-located pair resolves in its favour.
            expect(schemas.indexOf("root-redrob")).toBeGreaterThan(schemas.indexOf("root-legacy"))
          }).pipe(
            Effect.provide(
              testLayer(directory, global, root, {
                type: "git",
