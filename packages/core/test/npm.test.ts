...
describe("Npm.sanitize", () => {
  test("keeps normal scoped package specs unchanged", () => {
    expect(Npm.sanitize("@redrob/acme")).toBe("@redrob/acme")
    expect(Npm.sanitize("@redrob/acme@1.0.0")).toBe("@redrob/acme@1.0.0")
    expect(Npm.sanitize("prettier")).toBe("prettier")
  })

  test("handles git https specs", () => {
    const spec = "acme@git+https://github.com/redrob/acme.git"
    const expected = win ? "acme@git+https_//github.com/redrob/acme.git" : spec
