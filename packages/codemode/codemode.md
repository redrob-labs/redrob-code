...
This is the living design and status document for `@redrob-code/codemode` and its existing V2 OpenCode adapter.
...
`packages/redrob/src/tool/code-mode.ts`, where nested MCP calls run the `tool.execute.before` and
