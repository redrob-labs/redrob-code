# Redrob Code V2 Promise Plugin API

The Promise plugin API is the async/await equivalent of `@redrob-code/plugin/v2/effect`. It grants plugins the same two in-process capabilities:

- `hook` installs behavior at a Redrob Code extension point.
...
```ts
import { define } from "@redrob-code/plugin/v2/promise"
