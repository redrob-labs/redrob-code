...
function ref(request: HttpServerRequest.HttpServerRequest): Location.Ref {
  ...
  const workspaceID = query.get("location[workspace]") || request.headers["x-redrob-workspace"]
  const directory =
    query.get("location[directory]") ||
    (request.headers["x-redrob-directory"] ? decode(request.headers["x-redrob-directory"]) : process.cwd())
