...
import { getRedrobModeStack, REDROB_BASE_MODE, RedrobKeymapProvider, registerRedrobKeymap } from "../src/keymap"
...
test("legacy page key aliases compile as page keys", async () => {
  ...
  function Harness() {
    ...
    const offKeymap = registerRedrobKeymap(keymap, renderer, config)
...
test("mode-less bindings stay active when redrob mode changes", async () => {
  ...
  function Harness() {
    ...
    const popQuestion = getRedrobModeStack(keymap).push("question")
    counts.question = activeCounts()
    popQuestion()
    const popAutocomplete = getRedrobModeStack(keymap).push("autocomplete")
