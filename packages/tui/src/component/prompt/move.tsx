...
export function usePromptMove(input: { projectID: () => string | undefined; sessionID: () => string | undefined }) {
  ...
  async function create(context?: string) {
    ...
    if (!projectID) return
    setCreating(true)
    setProgress(language.t("prompt.move.creating_copy"))
    try {
      ...
      if (!directory) throw new Error(language.t("prompt.move.no_directory"))

      // Call a location-based route to make sure it's bootstrapped
      // before moving on
      await sdk.client.path.get({ directory }, { throwOnError: true })

      setProgress(language.t("prompt.move.creating_session"))
      return directory
    } catch (err) {
      homeDestination?.clear()
      setProgress(undefined)
      setCreating(false)
      toast.show({ title: language.t("prompt.workspace.create_failed"), message: errorMessage(err), variant: "error" })
      return
    }
  }
  ...
  async function moveExistingSession(sessionID: string, selection: MoveSessionSelection) {
    ...
    setProgress(language.t("prompt.move.moving"))
    try {
  ...
  function startSubmit() {
    if (progress()) setProgress(language.t("prompt.move.submitting"))
  }
