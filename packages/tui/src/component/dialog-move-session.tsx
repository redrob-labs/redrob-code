...
export function DialogMoveSession(props: DialogMoveSessionProps) {
  ...
  const options = createMemo<DialogSelectOption<MoveSessionSelection | undefined>[]>(() => {
    ...
    if (directories.loading && !data && !current)
      return [{ title: language.t("session.move.loading"), value: undefined }]
    const roots = [...(data ?? [])]
    ...
    if (roots.length === 0) return [{ title: language.t("session.move.empty"), value: undefined }]
    ...
    return list.map((item) => {
      ...
      return {
        ...
        titleView: isRemoving ? (
          <span style={{ fg: theme.error }}>{language.t("session.move.deleting", { path: item.location })}</span>
        ) : deleting ? (
          <span style={{ fg: theme.text }}>{language.t("dialog.delete_confirm", { shortcut: deleteHint() })}</span>
        ) : suffix ? (
          <>
    ...
    if (result.error) {
      ...
      if ("data" in result.error && result.error.data.forceRequired) {
        ...
        const choice = await DialogWorkspaceFileChanges.show(dialog, status?.data ?? [], {
          title: language.t("session.move.delete_copy.title"),
          message: language.t("session.move.delete_copy.message"),
        })
        if (choice !== "yes") {
          reopen()
          return
        ...
        if (forced.error) {
          toast.show({
            variant: "error",
            title: language.t("session.move.delete_copy_failed"),
  ...
  return (
    <box minHeight={showError() ? 5 : fullHeight()}>
      <DialogSelect
        title={language.t("session.move.title")}
        titleView={
          <box flexDirection="row" gap={1}>
            <text fg={theme.text} attributes={TextAttributes.BOLD}>
              {language.t("session.move.title")}
            </text>
          ...
          showError() ? (
            <box paddingLeft={4} paddingRight={4}>
              <text fg={theme.error} attributes={TextAttributes.BOLD}>
                {language.t("session.move.load_error")}
              </text>
