...
export const REDROB_BASE_MODE = "base"
export const COMMAND_PALETTE_COMMAND = "command.palette.show"

const REDROB_MODE_KEY = "redrob.mode"

export const RedrobKeymapProvider = KeymapProvider
export const useRedrobKeymap = useKeymap

export { useBindings, useKeymapSelector }

export type OpenTuiKeymap = ReturnType<typeof useKeymap>
type RedrobModeStack = ReturnType<typeof createRedrobModeStack>
...
const modeStacks = new WeakMap<OpenTuiKeymap, RedrobModeStack>()

function isVisiblePaletteCommand(command: Command) {
  return command.hidden !== true && command.name !== COMMAND_PALETTE_COMMAND
}

export function createRedrobModeStack(keymap: OpenTuiKeymap) {
  keymap.setData(REDROB_MODE_KEY, REDROB_BASE_MODE)

  const offFields = keymap.registerLayerFields({
    mode(value, ctx) {
      ctx.require(REDROB_MODE_KEY, value)
    },
  })
  ...
  const update = () => {
    keymap.setData(REDROB_MODE_KEY, stack.at(-1)?.mode ?? REDROB_BASE_MODE)
  }

  const stackApi = {
    current() {
      return stack.at(-1)?.mode ?? REDROB_BASE_MODE
    },
    push(mode: string) {
      if (disposed) return () => {}
    ...
    dispose() {
      ...
      stack.length = 0
      offFields()
      keymap.setData(REDROB_MODE_KEY, undefined)
...
export function useRedrobModeStack() {
  return getRedrobModeStack(useRedrobKeymap())
}

export function getRedrobModeStack(keymap: OpenTuiKeymap) {
  const value = modeStacks.get(keymap)
  if (!value) throw new Error("Redrob mode stack is not registered for this keymap")
  return value
}
...
export function registerRedrobKeymap(keymap: OpenTuiKeymap, renderer: CliRenderer, config: ResolvedKeymapConfig) {
  const modeStack = createRedrobModeStack(keymap)
