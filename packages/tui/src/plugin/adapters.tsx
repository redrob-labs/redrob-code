import type { TuiDialogSelectOption, TuiPluginApi, TuiSlotProps } from "@redrob-code/plugin/tui"
...
import type { useRedrobKeymap } from "../keymap"
...
type Input = {
  ...
  keymap: ReturnType<typeof useRedrobKeymap>
...
export function createTuiApiAdapters(input: Input): Omit<TuiPluginApi, "lifecycle"> {
  return {
    ...
    mode: {
      current() {
        return Keymap.getRedrobModeStack(input.keymap).current()
      },
      push(mode) {
        return Keymap.getRedrobModeStack(input.keymap).push(mode)
      },
    },
    route: {
      register(list) {
