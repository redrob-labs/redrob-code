...
const PLAIN_TEXT_FILETYPE = "redrob-plain-text"
...
function diffSourceLabel(language: LanguageContext, mode: DiffMode) {
  if (mode === "last-turn") return language.t("diff.source.last_turn")
  if (mode === "branch") return language.t("diff.source.branch")
  return language.t("diff.source.working_tree")
}
...
function DiffViewer(props: { api: TuiPluginApi }) {
  ...
  const commands = [
    {
      name: "diff.close",
      title: language.t("keybind.diff_close"),
      category: "VCS",
      run() {
    ...
    {
      name: "diff.down",
      title: language.t("diff.command.down"),
      category: "VCS",
      run: focusRunner({
        files() {
          moveFileSelection(1)
    ...
    {
      name: "diff.up",
      title: language.t("diff.command.up"),
      category: "VCS",
      run: focusRunner({
        files() {
          moveFileSelection(-1)
    ...
    {
      name: "diff.page.down",
      title: language.t("diff.command.page_down"),
      category: "VCS",
      run: focusRunner({
        files() {
          moveFileSelection(8)
    ...
    {
      name: "diff.page.up",
      title: language.t("diff.command.page_up"),
      category: "VCS",
      run: focusRunner({
        files() {
          moveFileSelection(-8)
    ...
    {
      name: "diff.toggle",
      title: language.t("keybind.diff_toggle"),
      category: "VCS",
      run: focusRunner({
        files() {
    ...
    {
      name: "diff.expand",
      title: language.t("keybind.diff_expand"),
      category: "VCS",
      run: focusRunner({
        files() {
          const highlighted = highlightedFileNode()
    ...
    },
    {
      ...
      title: language.t("keybind.diff_expand_all"),
      category: "VCS",
      run: focusRunner({
        files() {
          setExpandedFileNodes(allExpandedFileTreeDirectories(fileTree()))
    ...
    {
      name: "diff.collapse",
      title: language.t("keybind.diff_collapse"),
      category: "VCS",
      run: focusRunner({
        files() {
          const highlighted = highlightedFileNode()
    ...
    {
      name: "diff.next_hunk",
      title: language.t("keybind.diff_next_hunk"),
      category: "VCS",
      run() {
        jumpRelativeHunk(1)
      },
    },
    {
      ...
      title: language.t("keybind.diff_previous_hunk"),
      category: "VCS",
      run() {
        jumpRelativeHunk(-1)
      },
    ...
    {
      name: "diff.next_file",
      title: language.t("keybind.diff_next_file"),
      category: "VCS",
      run() {
    ...
    },
    {
      ...
      title: language.t("keybind.diff_previous_file"),
      category: "VCS",
      run() {
        jumpRelativePatchFile(-1)
      },
    },
    {
      ...
      title: language.t("diff.command.mark_reviewed"),
      category: "VCS",
      run() {
    ...
    },
    {
      ...
      title: language.t("keybind.diff_switch_focus"),
      category: "VCS",
      run() {
    ...
    },
    {
      ...
      title: language.t("keybind.diff_toggle_file_tree"),
      category: "VCS",
      run() {
    ...
    },
    {
      ...
      title: language.t("keybind.diff_single_patch"),
      category: "VCS",
      run() {
        setSelectedHunk(undefined)
        if (!singlePatch()) {
    ...
    },
    {
      ...
      title: language.t("keybind.diff_switch_source"),
      category: "VCS",
      run() {
    ...
    },
    {
      ...
      title: language.t("keybind.diff_toggle_view"),
      category: "VCS",
      run() {
    ...
    {
      name: "diff.help",
      title: language.t("keybind.diff_help"),
      category: "VCS",
      run() {
        openHelpDialog()
      },
  ...
  const switchDiffOptions = createMemo(() => {
    ...
    return [
      {
        title: language.t("diff.switch.working_tree"),
        value: "git" as const,
        description: language.t("diff.switch.working_tree.description"),
      },
      ...(vcs?.branch && vcs.default_branch && vcs.branch !== vcs.default_branch
        ? [
            {
              title: language.t("diff.switch.branch"),
              value: "branch" as const,
              description: language.t("diff.switch.branch.description"),
            },
          ]
        : []),
      {
        title: language.t("diff.switch.last_turn"),
        value: "last-turn" as const,
        description: language.t("diff.switch.last_turn.description"),
      },
    ]
  })
  ...
  const openSwitchDiffDialog = () => {
    props.api.ui.dialog.replace(() => (
      <DialogSelect
        title={language.t("diff.switch.title")}
        skipFilter={true}
        renderFilter={false}
        current={mode()}
  ...
  useBindings(() => ({
    commands,
    bindings: [
      { key: "j,down", cmd: "diff.down", desc: language.t("diff.command.down") },
      { key: "k,up", cmd: "diff.up", desc: language.t("diff.command.up") },
      { key: "pagedown,ctrl+f", cmd: "diff.page.down", desc: language.t("diff.command.page_down") },
      { key: "pageup,ctrl+b", cmd: "diff.page.up", desc: language.t("diff.command.page_up") },
      { key: "m", cmd: "diff.mark_reviewed", desc: language.t("diff.binding.mark_reviewed") },
    ...
    <box position="absolute" zIndex={2500} left={0} top={0} width={dimensions().width} height={dimensions().height}>
      <PanelGroup axis="y" width="100%" height="100%">
        <Panel border="none" flexShrink={0} padding={0} paddingLeft={1}>
          <text fg={theme().text}>{language.t("diff.title")} </text>
          <text fg={theme().textMuted}>{diffSourceLabel(language, mode())}</text>
          <box flexGrow={1} />
          <text fg={theme().textMuted}>{language.plural("diff.files", files().length)}</text>
        </Panel>
          ...
          <Switch>
            <Match when={diff.loading}>
              ...
              <box flexGrow={1} paddingLeft={1}>
                <text fg={theme().textMuted}>{language.t("diff.loading")}</text>
              </box>
            </Match>
            <Match when={!diff.loading && files().length === 0}>
              <Separator axis="x" />
              <box flexGrow={1} paddingLeft={1}>
                <text fg={theme().textMuted}>{language.t("diff.empty")}</text>
              </box>
            </Match>
            <Match when={!diff.loading && diff.error}>
              <Separator axis="x" />
              <box flexGrow={1} paddingLeft={1}>
                <text fg={theme().error}>{language.t("diff.error")}</text>
              </box>
            </Match>
            <Match when={!diff.loading}>
              <PanelGroup axis="x">
                        ...
                        return (
                          <box ref={(element: BoxRenderable) => registerPatchNode(entry.fileIndex, element)}>
                            ...
                            <Show
                              ...
                              fallback={<text fg={theme().textMuted}>{language.t("diff.no_patch")}</text>}
                            >
                              {(patch) => (
        ...
        <Panel flexShrink={0} gap={2} paddingLeft={1} border="none">
          <Show when={switchFocusShortcut()}>
            {(shortcut) => (
              <text fg={theme().text}>
                {shortcut()} <span style={{ fg: theme().textMuted }}>{language.t("diff.hint.focus_file_tree")}</span>
              </text>
            )}
          </Show>
          <Show when={nextFileShortcut()}>
            {(shortcut) => (
              <text fg={theme().text}>
                {shortcut()} <span style={{ fg: theme().textMuted }}>{language.t("diff.hint.next_file")}</span>
              </text>
            )}
          </Show>
          <Show when={nextHunkShortcut()}>
            {(shortcut) => (
              <text fg={theme().text}>
                {shortcut()} <span style={{ fg: theme().textMuted }}>{language.t("diff.hint.next_hunk")}</span>
              </text>
            )}
          </Show>
          <Show when={previousHunkShortcut()}>
            {(shortcut) => (
              <text fg={theme().text}>
                {shortcut()} <span style={{ fg: theme().textMuted }}>{language.t("diff.hint.previous_hunk")}</span>
              </text>
            )}
          </Show>
          <Show when={previousFileShortcut()}>
            {(shortcut) => (
              <text fg={theme().text}>
                {shortcut()} <span style={{ fg: theme().textMuted }}>{language.t("diff.hint.previous_file")}</span>
              </text>
            )}
          </Show>
          <Show when={switchSourceShortcut()}>
            {(shortcut) => (
              <text fg={theme().text}>
                {shortcut()} <span style={{ fg: theme().textMuted }}>{language.t("diff.hint.switch_source")}</span>
              </text>
            )}
          </Show>
          <Show when={markReviewedShortcut()}>
            {(shortcut) => (
              <text fg={theme().text}>
                {shortcut()} <span style={{ fg: theme().textMuted }}>{language.t("diff.hint.mark_reviewed")}</span>
              </text>
            )}
          </Show>
          <Show when={helpShortcut()}>
            {(shortcut) => (
              <text fg={theme().text}>
                {shortcut()} <span style={{ fg: theme().textMuted }}>{language.t("diff.hint.all")}</span>
              </text>
            )}
          </Show>
        </Panel>
...
function DiffViewerHelpDialog() {
  ...
  const language = useLanguage()
  const rows = [
    { shortcut: () => "q", name: "close" },
    { shortcut: useCommandShortcut("diff.switch_focus"), name: "switch_focus" },
    { shortcut: useCommandShortcut("diff.next_hunk"), name: "next_hunk" },
    { shortcut: useCommandShortcut("diff.previous_hunk"), name: "previous_hunk" },
    { shortcut: useCommandShortcut("diff.next_file"), name: "next_file" },
    { shortcut: useCommandShortcut("diff.previous_file"), name: "previous_file" },
    { shortcut: useCommandShortcut("diff.toggle_file_tree"), name: "toggle_file_tree" },
    { shortcut: useCommandShortcut("diff.single_patch"), name: "single_patch" },
    { shortcut: useCommandShortcut("diff.switch_source"), name: "switch_source" },
    { shortcut: useCommandShortcut("diff.toggle_view"), name: "toggle_view" },
    { shortcut: useCommandShortcut("diff.expand_all"), name: "expand_all" },
    { shortcut: useCommandShortcut("diff.mark_reviewed"), name: "mark_reviewed" },
  ] as const
    ...
    <box paddingLeft={2} paddingRight={2} paddingBottom={1} gap={1}>
      <box flexDirection="row" justifyContent="space-between">
        <text attributes={TextAttributes.BOLD} fg={theme.text}>
          {language.t("diff.help.title")}
        </text>
        <text fg={theme.textMuted}>{language.t("dialog.esc")}</text>
      </box>
      <box flexDirection="row">
        <text fg={theme.textMuted} width={5} wrapMode="none">
          {language.t("diff.help.column.key")}
        </text>
        <text fg={theme.textMuted} width={22} wrapMode="none">
          {language.t("diff.help.column.action")}
        </text>
        <text fg={theme.textMuted}>{language.t("diff.help.column.description")}</text>
      </box>
      <For each={rows}>
        {(row) => (
          <box flexDirection="row">
            ...
            <text fg={theme.text} width={22} wrapMode="none">
              {language.t(`diff.help.${row.name}.action`)}
            </text>
            <text fg={theme.textMuted}>{language.t(`diff.help.${row.name}.description`)}</text>
          </box>
        )}
      </For>
    </box>
    ...
    commands: [
      {
        ...
        get title() {
          return kvTranslator(api.kv).t("keybind.diff_open")
        },
        slashName: "diff",
        category: "VCS",
        namespace: "palette",
