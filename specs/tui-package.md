...
`packages/redrob/src/cli/cmd/tui` into a self-contained workspace package while
...
generic presentation libraries. It must not depend on `packages/redrob`,
`packages/cli`, or `@redrob-code/core`.
...
- Do not preserve private imports by creating aliases from `packages/tui` back
  into `packages/redrob`.
- Do not replace private `packages/redrob` imports with `@redrob-code/core`
...
- Keep legacy CLI command parsing, server startup, worker management,
  authentication, and config discovery outside `@redrob-code/tui`.

## Ownership Boundary

### `@redrob-code/tui` Owns
...
```text
packages/redrob/src/cli/cmd/tui
```

Its private dependency on `packages/redrob` is primarily expressed through
the `@/*` TypeScript alias, which resolves to `packages/redrob/src/*`.
...
Status: Completed. The private `@redrob-code/tui` workspace package now has an
...
- Add `packages/tui/package.json` with the name `@redrob-code/tui`.
...
- The package has no dependency on `opencode`, `@redrob-code/cli`, or
  `@redrob-code/core`.
...
primitives now live in `@redrob-code/tui`. The legacy host consumes explicit
...
and build inputs through `@redrob-code/tui/runtime`. Movable app, component,
...
`@redrob-code/tui/config` now owns schemas, defaults, keybind resolution, the
...
- Keep config path discovery, project/global precedence, migration, variable
  expansion, and plugin package installation in `packages/redrob` initially.
...
canonical in `@redrob-code/tui`. Configured references resolve through the new
...
`@redrob-code/tui`. The legacy host injects a narrow plugin host that retains
...
- Move plugin rendering tests to `packages/tui`; retain installation/loading
  integration tests in `packages/redrob`.
...
- The package has no imports from `packages/redrob`, `packages/cli`, or
  `@redrob-code/core`.
...
public `@redrob-code/tui` root while retaining worker/server/config/plugin and
...
- Keep the legacy yargs commands corresponding to current `thread.ts` and
  `attach.ts` in `packages/redrob`.
- Keep the legacy embedded worker and server startup in `packages/redrob`.
- Change those adapters to load config, create transport inputs, and call the
  public `@redrob-code/tui` API.
...
- Remove obsolete `@tui/*` path mappings from `packages/redrob`.
- Remove stale test fixtures and update all imports to package exports.
- Narrow `@redrob-code/tui` exports to intentional public entrypoints.
...
- No source under `packages/tui` imports `@/...`, `@redrob-code/core`, or either
  executable package.
...
cd packages/redrob && bun typecheck
...
rg '@redrob-code/core|packages/redrob|packages/cli' packages/tui
rg 'src/cli/cmd/tui|@tui/' packages/redrob/src packages/redrob/test
```
...
- Build the current-platform `packages/redrob` binary.
